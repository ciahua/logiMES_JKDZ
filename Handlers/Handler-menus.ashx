...
public class Handler_planreport : IHttpHandler
{
    ...
    {
        string sql = "select id, pId as pid, name, url from Menus";
    ...
    private string AddSubLine2Json(string da)
    {
        ...
        //获取最大的id值
        string sqlMaxId = string.Format("SELECT MAX(id) FROM dbo.Menus");
        ...
        string sql = string.Format("insert into Menus values('{0}','{1}','{2}', '{3}')", ++maxId, jo.id, "请输入新的子菜单名称", "");
    ...
    private string AddNewLine2Json(string da)
    {
        ...
        if (maxIdObj != null)
        {
            maxId = maxIdObj==DBNull.Value ? 1000 : Convert.ToInt32(maxIdObj);
        }
        string sql = string.Format("insert into Menus values('{0}','{1}','{2}', '{3}')", ++maxId, "0", "请输入新的菜单名称", "");
        bool isOk = DbHelperSQL.Execute(sql, connStr);
        string s = string.Empty;
        if (isOk)
        {
    ...
    {
        Children jo = JsonConvert.DeserializeObject<Children>(da);
        string sqlDel = string.Format("delete from menus where id='{0}' and pid='{1}'", jo.id, jo.pid);
    ...
    {
        Children jo = JsonConvert.DeserializeObject<Children>(da);
        string sqlDel = string.Format("update Menus set name='{0}' where id='{1}' and pid='{2}'", nv, jo.id, jo.pid);
        bool isOk = DbHelperSQL.Execute(sqlDel, connStr);
        string s = string.Empty;
        if (isOk)
        {
