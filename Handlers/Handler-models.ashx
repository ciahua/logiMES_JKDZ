...
public class Handler_planreport : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (actId)
        {
            ...
            case "insTable":
                ...
                //string sqlAddNew = string.Format("insert into models values('{0}','{1}','{2}')", m["model"], m["desc"], t);
                string sqlAddNew = $"if not exists (select 1 from models where model='{m["model"].ToString().Trim()}') " +
                    $"insert into models values('{m["model"].ToString().Trim()}', '{m["desc"].ToString().Trim()}', '{t}') " +
                    $"else " +
                    $"update models set Describe='{m["desc"].ToString().Trim()}', ChangeTime='{t}' where Model='{m["model"]}' ";
            ...
            case "insFileTable":
                ...
                {
                    src = mf["src"].ToString().Trim();
                }
                string sqlAddNewFile = $"insert into modelfiles values('{mf["model"].ToString().Trim()}', '{mf["station"].ToString().Trim()}', '{mf["filename"].ToString().Trim()}', '{mf["desc"].ToString().Trim()}', '{src}', '{mf["url"].ToString().Trim()}', '{tf}') ";
            ...
            case "updateTable":
                ...
                string sqlEditRow = string.Format("update Models set Model='{0}', Describe = '{1}', ChangeTime='{2}' where Id="+mod["id"], mod["model"], mod["desc"], ct);
            ...
            case "delTable":
                ...
                string sqlDelRow = string.Format("delete from Models where Id=" + md["Id"]);
                ...
                break;
            case "delFileTable":
                JObject mft = (JObject)JsonConvert.DeserializeObject(modData);
                string sqlDelFile = string.Format("delete from ModelFiles where Id=" + mft["Id"]);
                bool sqlDelFileResult = DbHelperSQL.Execute(sqlDelFile, connStr);
                if (sqlDelFileResult)
                {
                    resJson = "success";
                }
    ...
    private string GetTable2Json()
    {
        ...
        sql = "SELECT * from (SELECT *, ROW_NUMBER() OVER(ORDER BY Id ASC) AS Num FROM Models)AS TempTable WHERE Num BETWEEN " + startIndex + " AND " + endIndex;
